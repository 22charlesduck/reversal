...
def evaluate(model, loader, ctx, temperature, top_k, results=None, mode='test'):
    ...
    """
    num_prefix_tokens = loader.dataset.num_eval_prefix
    num_target_tokens = loader.dataset.num_eval_target
    ...
    for x in bar:
        ...
        correct = y.eq(y_pred[:, num_prefix_tokens:num_prefix_tokens+num_target_tokens]).float()
