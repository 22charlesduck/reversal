...
parser.add_argument(
        "--save_every", type=int, default=40, help="Interval (in steps) at which to save model",
    )
parser.add_argument(
...
# Setup wandb logging
if wandb_log:
    wandb.init(project='reversal', entity=wandb_entity, config=args.__dict__,)
...
for ep in range(args.epochs):
    ...
    for x, y, attn_mask in train_bar:
        ...
        if ep % args.save_every == 0 and ep > 0:
            ...
            checkpoint_path = os.path.join(checkpoint_dir, "model_checkpoint_epoch_{}.pt".format(ep))
